...
//let se puede reasignar el valor de una varieble
function foo() {
    let i = 0;
    if(true) {
        i = 1;
...
var edad_Persona1 = 50; 
var edad_Persona2 = 066;
...
if (helado === 'chocolate') {
  ...
  alert('Mi favorito es el de chocolate...');    
}
...
// Ejemplo de throw
 function getNombreMes(mes) {
  ...
  try {
     ...
     registrarMisErrores(excepcion.mensaje, excepcion.nombre); //Capturador de errores
  }
...
var pet = function(name) {          // Funcion externa
      var getName = function() {
        return name;                // Funcion interna 
      }

      return getName;               // Qué devuelve
    },
    myPet = pet("Vivie");
...
function myConcat(separator) {
 ...
 function multiply(a, b) {
  ...
  eval(cadenadecaracteresostring); //Método eval
  isFinite(number); //Funcion global isFinite
  if(isFinite(ClientInput)){
  }
  var floatValue = parseFloat(toFloat); //Funcion isNaN
  if (isNaN(floatValue)) {
     notFloat();
    } else {
   isFloat();
  }
parseFloat(str); //Funciones parseInt and parseFloat
parseInt(str [ radix]);
